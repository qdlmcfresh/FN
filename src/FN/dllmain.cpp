...
void init(HMODULE hModule)
{
	...
	while (!GetAsyncKeyState(VK_DELETE) & 1)
	{
		...
		/*if (pLocalPlayerPawn->CurrentWeapon)
		{
			AFortWeaponRanged *pLocalPlayerWeaponRanged = static_cast<AFortWeaponRanged*>(pLocalPlayerPawn->CurrentWeapon);
			if (pLocalPlayerWeaponRanged)
			{
				if (pLocalPlayerWeaponRanged->CurrentNumBullets > 0)
				{
					pLocalPlayerWeaponRanged->CurrentReticleSpread = 0.f;
					pLocalPlayerWeaponRanged->CurrentReticleSpreadMultiplier = 0.f;
					pLocalPlayerWeaponRanged->CurrentStandingStillSpreadMultiplier = 0.f;
					pLocalPlayerWeaponRanged->CurrentCrouchingSpreadMultiplier = 0.f;
				}
			}
		}*/
		...
		for (int i = 0; i < pRealWorld->PersistentLevel->AActors.Num(); i++)
		{
			...
			if (pPlayerPawn == pLocalPlayerPawn)
				continue;
